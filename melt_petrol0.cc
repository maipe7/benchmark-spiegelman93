        ...
        evaluate(const typename Interface<dim>::MaterialModelInputs &in, typename Interface<dim>::MaterialModelOutputs &out) const
    {
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
      {
        ...
        if (melt_out != nullptr)
        {
          ...
          melt_out->compaction_viscosities[i] = eta_b0; //out.viscosities[i] * (2.0 + 0.3 / std::max(porosity, 3e-4)); // ref
        }
    ...
    MeltPetrol<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Melt petrol");
        {
          ...
          prm.declare_entry("Reference bulk viscosity", "5e20",
                            Patterns::Double(0.),
                            "The value of the bulk viscosity $\\eta_b0$ of the solid matrix. "
    ...
    MeltPetrol<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Melt petrol");
        {
          ...
          eta_b0 = prm.get_double("Reference bulk viscosity");
